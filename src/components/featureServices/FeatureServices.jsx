      ...
      <div className="relative max-w-7xl mx-auto px-4 z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-spacing-1">
          {services.map((service, index) => (
            <motion.div
              key={index}
              className="bg-light shadow-md rounded-lg p-spacing-1 flex flex-col justify-between"
              initial={{ opacity: 0, y: 50 }}
              animate={isVisible ? { opacity: 1, y: 0 } : {}}  // Trigger animation when visible
            ...
            >
              <div className="flex flex-col items-center">
                ...
                />
                <h3 className="font-secondary font-bold text-size-3 text-primary mb-spacing-1 text-center">
                  {service.title}
                </h3>
                <p className="font-secondary text-size-1 text-dark mb-4 pb-spacing-2 text-center">
                  {service.description}
                </p>
              </div>
              <div className="flex justify-center mt-auto">
                <a
                  href={service.link}
                  className="inline-block bg-primary text-light font-bold px-6 py-3 rounded-full btn-custom hover:bg-accent hover:text-primary"
                >
                  Más Información
                </a>
              </div>
